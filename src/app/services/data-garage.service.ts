...
export class DataGarageService {
  ...
  async getLastTransactions(quantity = 5) {
    ...
    const filteredTransactions = this.parkingsDataService.garages.filter(garage => 
      garage.entryHour !== null && garage.exitHour !== undefined
    );

    const lastTransactions = filteredTransactions
        .sort((a, b) => new Date(b.entryHour.replace(" ", "T")).getTime() - new Date(a.entryHour.replace(" ", "T")).getTime()) // Ordenar de más reciente a más antiguo
        .slice(0, quantity);
